...
<style>
    ...
    @media (max-width: 768px) {
        ...
        .topbar-sticky { padding-left: 56px !important; flex-wrap: wrap; gap: 10px !important; row-gap: 8px !important; }
...
<main class="app-main" style="flex:1; margin-left:240px; display:flex; flex-direction:column; min-height:100vh;">
    ...
    <div class="content-area" style="padding:28px; max-width:900px; width:100%;">
        ...
        <div style="background:#FEF2F2; border:1px solid #FECACA; border-radius:10px; padding:14px 18px; margin-bottom:20px; display:flex; gap:12px; align-items:flex-start;">
            ...
            <div>
                ...
                <div style="font-size:12px; color:#B91C1C; margin-top:2px;">• {{ $error }}</div>
                @endforeach
            </div>
        </div>
        @endif
        ...
        <div style="background:white; border-radius:12px; border:1px solid #E2E8F0; overflow:hidden; margin-bottom:14px; box-shadow:0 1px 4px rgba(0,0,0,0.05);">
            ...
            <form method="POST" action="{{ route('pengaturan.update') }}" enctype="multipart/form-data" style="padding:20px;">
                @csrf
                ...
                <div style="display:flex; align-items:center; gap:16px; margin-bottom:18px; padding:14px 16px; background:#F8FAFC; border-radius:10px; border:1px solid #F1F5F9; flex-wrap:wrap;">
                    ...
                    <div>
                        <div style="font-size:14px; font-weight:600; color:#0F172A;">{{ Auth::user()->name }}</div>
            ...
            <div class="info-grid-3" style="padding:16px 20px; display:grid; grid-template-columns:repeat(3,1fr); gap:12px;">
                ...
                <div style="background:#F8FAFC; border-radius:10px; padding:12px 14px; border:1px solid #F1F5F9;">
                    ...
                    <div style="font-size:14px; font-weight:600; color:#0F172A;">
                        {{ $school->radius ?? '-' }}
                        @if($school?->radius)<span style="font-size:12px; color:#64748B; font-weight:400;"> meter</span>@endif
                    </div>
                </div>
                <div style="background:#F8FAFC; border-radius:10px; padding:12px 14px; border:1px solid #F1F5F9;">
                    ...
                    <div style="font-size:13px; font-weight:600; color:#0F172A; font-family:monospace;">
                        {{ $school->latitude ?? '-' }},<br>{{ $school->longitude ?? '-' }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="two-col-fields" style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:6px;">
