        ...
        <div style="background:white; border:1px solid #E2E8F0; border-top:3px solid #3B82F6; border-radius:12px; padding:16px 18px; box-shadow:0 1px 4px rgba(0,0,0,0.05);">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    ...
                    <div style="font-size:26px; font-weight:700; color:#1E293B; margin-top:4px;">{{ $schools->count() }}</div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    ...
                    <div style="font-size:26px; font-weight:700; color:#1E293B; margin-top:4px;">
                        {{ $schools->count() ? round($schools->avg('radius')) : 0 }} m
                    </div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    ...
                    <div style="font-size:15px; font-weight:600; color:#1E293B; margin-top:6px;">
                        {{ $schools->count() ? $schools->sortByDesc('updated_at')->first()->updated_at->diffForHumans() : '-' }}
                    </div>
                </div>
        ...
        <table style="width:100%; border-collapse:collapse; font-size:13px;">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #F1F5F9;" onmouseover="this.style.background='#F8FAFF'" onmouseout="this.style.background='white'">
                    ...
                    {{-- Nama Sekolah --}}
                    <td style="padding:14px 16px;">
                        <div style="font-size:14px; font-weight:600; color:#1E293B;">{{ $s->nama_sekolah }}</div>
                    </td>
                    ...
                    <td style="padding:14px 16px; text-align:center;">
                        <div style="display:flex; align-items:center; justify-content:center; gap:8px;">
                            ...
                            <form action="{{ route('superadmin.schools.delete', $s->id) }}" method="POST" style="display:inline;"
                                  onsubmit="return confirm('Hapus {{ $s->nama_sekolah }}? Tindakan ini tidak dapat dibatalkan.')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
